...
def dataCheck(doc_id):
    ...
    if WEIGHT_VAR not in variables: 
       WARN_VS=mark_safe(WARN_VS+"<p style='color:red'> The variable " +BMI_VAR+ " is missing </p>")
    if (HEIGHT_VAR not in variables) & (BMI_VAR not in variables) & (WEIGHT_VAR not in variables) :
       WARN_VS= "<p style='color:red'> Height Weight and BMI are not available </p>" 
    ...
    res=[WARN_FL,WARN_VS,WARN_CDISC,HEIGHT_VAR,WEIGHT_VAR,BMI_VAR]
...
def data_dash2(request, doc_id  ):
    ...
    age_f0 = [] 
    age_m0 = []  
    bgcolor_arm = ['rgba(0, 0, 255, 0.3)','rgba(255, 99, 132, 0.3)','rgba(0, 255, 0, 0.3)','yellow']	
    bocolor_arm = ['rgba(0, 0, 255, 1)','rgba(255, 99, 132, 1)','rgba(0, 255, 0, 1)','yellow']	
    ...
    for  num,val in  enumerate(armcd, start=1):
        ...
        if num == 1 :
           df.loc[df['ARMCD'] == val, 'ARM'] = mark_safe('<span class="label label-success">' + val)
        if num == 2 :
           df.loc[df['ARMCD'] == val, 'ARM'] = mark_safe('<span class="label label-warning">' + val)
        if num == 3 :
           df.loc[df['ARMCD'] == val, 'ARM'] = mark_safe('<span class="label label-info">' + val)
        if num == 4 :
           df.loc[df['ARMCD'] == val, 'ARM'] = mark_safe('<span class="label label-danger">' + val)
		
        if 	WARN_MESSAGE[1] == '' :
        
            dftmp=df[(df['ARMCD'] == val)]
            WTMP = dftmp[[WARN_MESSAGE[3],WARN_MESSAGE[4],WARN_MESSAGE[5]]].to_json(orient='records').replace('"','').replace(WARN_MESSAGE[3],"y").replace(WARN_MESSAGE[5],"r").replace(WARN_MESSAGE[4],"x")
    ...
    return render(request, 'figures/data_dash2.html', { 'document':doc,'bar_male':bar_male, 'bar_female':bar_female, 'bar_labels':bar_labels,'box_labels':box_labels,'age_f':age_f,'age_m':age_m,'listing':listing[:10] ,'WARN_MESSAGE':WARN_MESSAGE,'FLAG1':flag1,'FLAG2':flag2,'C_FLAG1':c_flag1,'C_FLAG2':c_flag2,'P_FLAG1':p_flag1,'P_FLAG2':p_flag2,'WV':WV} )   
